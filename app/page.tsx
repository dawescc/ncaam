...
function Row({ awayScore, color, date, homeScore, index, isLast, logo, name, rank, teamId, winner }: any) {
	return (
		<div className={clsx("flex flex-col min-[450px]:flex-row justify-between px-0 py-2", { "border-b border-gray-200 dark:border-gray-800": !isLast })}>
			<div className='flex'>
				<Image
					src={logo}
					alt={name}
				...
				<Link
					...
					{rank !== 99 ? <span className='text-sm uppercase font-normal text-gray-500 mr-2'>{rank}</span> : null}
					{name}
				</Link>
			</div>
			<div className='flex flex-row-reverse justify-end min-[450px]:flex-row'>
				{homeScore ? <p className='text-gray-700 dark:text-gray-300 tabular-nums'>{`${homeScore}-${awayScore}`}</p> : null}
				{homeScore ? (
					winner ? (
					...
					<p className='font-semibold text-gray-500 ml-0 min-[450px]:ml-2 w-5 mr-4 min-[450px]:mr-0 text-center'>—</p>
				) : (
...
async function Schedule() {
	...
	const [team, allTeams] = await Promise.all([getTeamData("2633"), getAllTeamIds()]);
		...
		<section className='w-full mx-auto p-6'>
			<div className='flex items-center'>
				...
				{nextGame?.selectedTeamRank !== 99 ? (
					<span className='text-sm uppercase font-normal text-gray-500 ml-2'>{nextGame?.selectedTeamRank}</span>
				) : null}
			...
			<h3 className='font-semibold text-gray-700 dark:text-gray-300 mt-4'>Full</h3>
			<div>
